...
function stringify(map: EnvMap): string {
    const lines: string[] = []
    for (const [k, v] of Object.entries(map).sort(([a], [b]) =>
        a.localeCompare(b)
    )) {
        if (needsQuote(v)) {
...
function mergeEnv(tfValues: EnvMap, targetPath: string): void {
    ...
    console.log(
        `Wrote ${targetPath} (${Object.keys(merged).length} keys total)`
    )
...
if (!envArg || !VALID_ENVS.includes(envArg as (typeof VALID_ENVS)[number])) {
    console.error(
        envArg
