...
for (const arg of process.argv.slice(2)) {
    if (arg.startsWith('--env=')) {
        ...
        if (val !== 'staging' && val !== 'production') {
            console.error(
                `Invalid environment: ${val} (must be staging or production)`
            )
            process.exit(1)
        }
        env = val
...
if (!prefix) {
    console.error(
        'Could not read functionPrefix from terraform-scaffold.config.ts'
    )
    process.exit(1)
}
...
if (config.lambdaBuild?.aliases) {
    for (const [key, value] of Object.entries(
        config.lambdaBuild.aliases as Record<string, string>
    )) {
        aliases[key] =
            value === '.'
                ? process.cwd()
...
const external = config.lambdaBuild?.external ?? [
    'aws-sdk',
    '@aws-sdk/*',
    '@aws-appsync/utils',
]
...
async function zipFunction(bundlePath: string, zipPath: string): Promise<void> {
    ...
    writeFileSync(
        packageJsonPath,
        JSON.stringify(
            { version: '0.0.1', lastBuildAt: formatTimestamp() },
            null,
            2
        ) + '\n'
    )
