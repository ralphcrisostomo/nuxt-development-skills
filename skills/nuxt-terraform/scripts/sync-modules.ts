...
function writeManifest(version: string, checksums: Record<string, string>): void {
...
for (const [file, sourceHash] of Object.entries(sourceChecksums)) {
  ...
  if (!targetHash) {
    ...
    if (manifest?.checksums[file] && manifest.checksums[file] !== targetHash) {
...
if (locallyModified.length === 0 && needsUpdate.length === 0 && newFiles.length === 0) {
...
if (force && locallyModified.length > 0) {
  ...
  console.log(`  Overwrote ${locallyModified.length} locally modified file(s)`);
}
...
if (needsUpdate.length > 0) console.log(`  Updated ${needsUpdate.length} file(s)`);
