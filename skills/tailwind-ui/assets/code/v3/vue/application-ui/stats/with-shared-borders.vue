<template>
  <div>
    ...
    <dl class="mt-5 grid grid-cols-1 divide-gray-200 overflow-hidden rounded-lg bg-white shadow md:grid-cols-3 md:divide-x md:divide-y-0 dark:divide-white/10 dark:bg-gray-800/75 dark:shadow-none dark:ring-1 dark:ring-inset dark:ring-white/10">
      <div v-for="item in stats" :key="item.name" class="px-4 py-5 sm:p-6">
        ...
        <dd class="mt-1 flex items-baseline justify-between md:block lg:flex">
          ...
          <div :class="[item.changeType === 'increase' ? 'bg-green-100 text-green-800 dark:bg-green-400/10 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-400/10 dark:text-red-400', 'inline-flex items-baseline rounded-full px-2.5 py-0.5 text-sm font-medium md:mt-2 lg:mt-0']">
            <ArrowUpIcon v-if="item.changeType === 'increase'" class="-ml-1 mr-0.5 size-5 shrink-0 self-center text-green-500 dark:text-green-400" aria-hidden="true" />
            <ArrowDownIcon v-else class="-ml-1 mr-0.5 size-5 shrink-0 self-center text-red-500 dark:text-red-400" aria-hidden="true" />
