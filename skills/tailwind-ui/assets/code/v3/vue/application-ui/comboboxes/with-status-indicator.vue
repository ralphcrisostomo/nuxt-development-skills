  ...
  <Combobox as="div" v-model="selectedPerson" @update:modelValue="query = ''">
    ...
    <div class="relative mt-2">
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="" leave-to-class="opacity-0">
        <ComboboxOptions v-if="filteredPeople.length > 0 || query.length > 0" class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg outline outline-1 outline-black/5 sm:text-sm dark:bg-gray-800 dark:shadow-none dark:-outline-offset-1 dark:outline-white/10">
          <ComboboxOption v-if="queryPerson" :value="queryPerson" as="template" v-slot="{ active }">
            <li :class="['relative cursor-default select-none px-3 py-2', active ? 'bg-indigo-600 text-white outline-none dark:bg-indigo-500' : 'text-gray-900 dark:text-white']">
              <div class="flex items-center">
                <span class="inline-block size-2 shrink-0 rounded-full border border-gray-400 dark:border-gray-500 [[aria-active='true']_&]:border-white/75" aria-hidden="true" />
