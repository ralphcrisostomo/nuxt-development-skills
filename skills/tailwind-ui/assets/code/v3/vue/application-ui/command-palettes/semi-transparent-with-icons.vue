    ...
    <Dialog class="relative z-10" @close="open = false">
      ...
      <div class="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:p-20">
        <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 scale-95" enter-to=" scale-100" leave="ease-in duration-200" leave-from=" scale-100" leave-to="opacity-0 scale-95">
          <DialogPanel class="mx-auto max-w-2xl transform divide-y divide-gray-500/10 overflow-hidden rounded-xl bg-white/80 shadow-2xl outline outline-1 outline-black/5 backdrop-blur backdrop-filter transition-all dark:divide-white/10 dark:bg-gray-900/80 dark:-outline-offset-1 dark:outline-white/10">
            <Combobox @update:modelValue="onSelect">
              <div class="grid grid-cols-1">
                <ComboboxInput class="col-start-1 row-start-1 h-12 w-full bg-transparent pl-11 pr-4 text-base text-gray-900 outline-none placeholder:text-gray-500 sm:text-sm dark:text-white dark:placeholder:text-gray-400" placeholder="Search..." @change="query = $event.target.value" @blur="query = ''" />
              ...
              <ComboboxOptions v-if="query === '' || filteredProjects.length > 0" static as="ul" class="max-h-80 scroll-py-2 divide-y divide-gray-500/10 overflow-y-auto dark:divide-white/5">
                <li class="p-2">
                  <h2 v-if="query === ''" class="mb-2 mt-4 px-3 text-xs font-semibold text-gray-900 dark:text-white">Recent searches</h2>
                  <ul class="text-sm text-gray-700 dark:text-gray-300">
                    <ComboboxOption v-for="project in query === '' ? recent : filteredProjects" :key="project.id" :value="project" as="template" v-slot="{ active }">
                      <li :class="['flex cursor-default select-none items-center rounded-md px-3 py-2', active && 'bg-gray-900/5 text-gray-900 outline-none dark:bg-white/5 dark:text-white']">
