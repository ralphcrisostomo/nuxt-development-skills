    ...
    <Dialog class="relative z-10" @close="open = false">
      ...
      <div class="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:p-20">
        <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 scale-95" enter-to=" scale-100" leave="ease-in duration-200" leave-from=" scale-100" leave-to="opacity-0 scale-95">
          <DialogPanel class="mx-auto max-w-xl transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white shadow-2xl outline outline-1 outline-black/5 transition-all dark:divide-white/10 dark:bg-gray-900 dark:-outline-offset-1 dark:outline-white/10">
            <Combobox @update:modelValue="onSelect">
              <div class="grid grid-cols-1">
                <ComboboxInput class="col-start-1 row-start-1 h-12 w-full pl-11 pr-4 text-base text-gray-900 outline-none placeholder:text-gray-400 sm:text-sm dark:bg-gray-900 dark:text-white dark:placeholder:text-gray-500" placeholder="Search..." @change="rawQuery = $event.target.value" @blur="query = ''" />
                ...
                <li v-if="filteredProjects.length > 0">
                  ...
                  <ul class="-mx-4 mt-2 text-sm text-gray-700 dark:text-gray-300">
                    <ComboboxOption v-for="project in filteredProjects" :key="project.id" :value="project" as="template" v-slot="{ active }">
                      <li :class="['flex cursor-default select-none items-center px-4 py-2', active && 'bg-indigo-600 text-white outline-none dark:bg-indigo-500']">
                ...
                <li v-if="filteredUsers.length > 0">
                  ...
                  <ul class="-mx-4 mt-2 text-sm text-gray-700 dark:text-gray-300">
                    <ComboboxOption v-for="user in filteredUsers" :key="user.id" :value="user" as="template" v-slot="{ active }">
                      <li :class="['flex cursor-default select-none items-center px-4 py-2', active && 'bg-indigo-600 text-white outline-none dark:bg-indigo-500']">
                        <img :src="user.imageUrl" alt="" class="size-6 flex-none rounded-full bg-gray-100 outline outline-1 -outline-offset-1 outline-black/5 dark:bg-gray-800 dark:outline-white/10" />
              ...
              <div class="flex flex-wrap items-center bg-gray-50 px-4 py-2.5 text-xs text-gray-700 dark:bg-gray-800/50 dark:text-gray-300">
                Type
                <kbd :class="['mx-1 flex size-5 items-center justify-center rounded border bg-white font-semibold sm:mx-2 dark:bg-gray-800', rawQuery.startsWith('#') ? 'border-indigo-600 text-indigo-600 dark:border-indigo-500 dark:text-indigo-500' : 'border-gray-400 text-gray-900 dark:border-white/10 dark:text-white']">#</kbd>
                <span class="sm:hidden">for projects,</span>
                <span class="hidden sm:inline">to access projects,</span>
                <kbd :class="['mx-1 flex size-5 items-center justify-center rounded border bg-white font-semibold sm:mx-2 dark:bg-gray-800', rawQuery.startsWith('>') ? 'border-indigo-600 text-indigo-600 dark:border-indigo-500 dark:text-indigo-500' : 'border-gray-400 text-gray-900 dark:border-white/10 dark:text-white']">&gt;</kbd>
                for users, and
                <kbd :class="['mx-1 flex size-5 items-center justify-center rounded border bg-white font-semibold sm:mx-2 dark:bg-gray-800', rawQuery === '?' ? 'border-indigo-600 text-indigo-600 dark:border-indigo-500 dark:text-indigo-500' : 'border-gray-400 text-gray-900 dark:border-white/10 dark:text-white']">?</kbd>
                for help.
              </div>
            </Combobox>
          </DialogPanel>
