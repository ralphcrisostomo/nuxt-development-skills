...
| **Commit Delegation** | Delegates the commit to `git-commit-skill` for Sensitive File Guard, Conventional Commits, and heredoc format. |
...
```bash
# For Claude Code
mkdir -p .claude/skills/git-squash-skill
cp SKILL.md .claude/skills/git-squash-skill/

# Or if using .agents/skills/ as source of truth
mkdir -p .agents/skills/git-squash-skill
cp SKILL.md .agents/skills/git-squash-skill/
```
...
This skill requires **git-commit-skill** to be installed in the same project. The commit step is fully delegated — `git-squash-skill` never writes commit messages directly.

```bash
# Ensure git-commit-skill is also installed
ls .claude/skills/git-commit-skill/SKILL.md
```

## Workflow overview

```
Pre-flight → checkout main → git merge --squash → /git-commit-skill → verify → cleanup
```
...
4. **Commit** — delegate to `/git-commit-skill` (guard, stage, message, heredoc, hooks)
...
/git-squash-skill feature/my-branch
```
